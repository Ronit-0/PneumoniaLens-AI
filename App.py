    ...
    if not st.session_state.logged_in_doctor:
        ...
        if uploaded_file:
            ...
            def generate_pdf():
                ...
                c.drawString(50, height - 60, "PneumoniaLens AI Diagnostic Report")
                
                c.setFont("Helvetica", 12)
                c.drawString(50, height - 90, f"Operator: Dr. {st.session_state.doctor_name}")
                c.drawString(50, height - 110, f"Date & Time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
                
                c.setFont("Helvetica-Bold", 12)
                c.drawString(50, height - 140, f"AI Diagnosis: {final_label}")
                c.drawString(50, height - 160, f"Confidence Score: {confidence*100:.2f}%")
                
                # --- 2. Injecting the Images ---
                # Convert the OpenCV Grad-CAM overlay to a PIL Image for the PDF
                ...
                c.drawImage(img_original, 50, height - 420, width=230, height=230, preserveAspectRatio=True)
                c.drawImage(img_heatmap, 310, height - 420, width=230, height=230, preserveAspectRatio=True)
                
                c.setFont("Helvetica-Oblique", 10)
                c.drawString(110, height - 440, "Original X-Ray")
                c.drawString(350, height - 440, "Grad-CAM Heatmap Focus")
                
                # --- 3. Dynamic Clinical AI Findings ---
                c.setFont("Helvetica-Bold", 14)
                c.drawString(50, height - 490, "Automated AI Findings:")
                
                # Generate specific text based on the result
                if final_label == "NORMAL":
                    findings = "The DenseNet121 model analyzed the thoracic cavity and detected no significant opacities or consolidations indicative of pneumonia. The lung fields appear generally clear within the analyzed parameters."
                elif final_label == "PNEUMONIA_BACTERIA":
                    findings = "The model detected focal consolidations and dense lobar opacities highly indicative of a bacterial infection. The Grad-CAM heatmap highlights the specific localized regions driving this high-confidence classification."
                elif final_label == "PNEUMONIA_VIRAL":
                    findings = "The model detected diffuse, patchy, or ground-glass opacities characteristic of a viral respiratory infection. The Grad-CAM heatmap localizes the distributed bilateral patterns typical of viral pneumonia."
                else:
                    findings = "The AI model was unable to reach the required confidence threshold. Manual radiologist review is strongly advised to determine the exact pathology."
                
                # Wrap the findings text so it doesn't go off the page
                text_obj = c.beginText(50, height - 515)
                text_obj.setFont("Helvetica", 11)
                wrapped_text = textwrap.wrap(findings, width=85)
                for line in wrapped_text:
                    text_obj.textLine(line)
                c.drawText(text_obj)
                
                # --- 4. The Medical Disclaimer Footer ---
                c.setFont("Helvetica-Bold", 10)
                c.drawString(50, 100, "CLINICAL DISCLAIMER:")
